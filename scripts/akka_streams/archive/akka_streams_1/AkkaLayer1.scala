...
object AkkaLayer1 {
  implicit val system: ActorSystem = ActorSystem("neuron-final-system")
  ...
  val consumerSettings = ConsumerSettings(system, new StringDeserializer, new StringDeserializer)
    ...
    .withGroupId("neuron-final-group")
  ...
  def writeToRedis(imageId: String, outputs: Array[Double]): Future[Unit] = Future {
    ...
    try {
      ...
      jedis.hset("streams:predictions", imageId, maxIndex.toString)
    } finally {
      jedis.close()
    }
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    val kafkaGraph = Consumer
      ...
      .map { imageId =>
        val record = new ProducerRecord[String, String]("layer-output", imageId, imageId)
