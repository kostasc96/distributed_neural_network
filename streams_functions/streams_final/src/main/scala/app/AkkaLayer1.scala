...
object AkkaLayer1 extends App {
  ...
  // Load Lua script for Redis operations
  private val script =
    s"""
       | local blobKey = KEYS[1]
       | local countKey = KEYS[2]
       | local neuronCount = ${neuronCount}
       | local ttlSec = ${ttl.toSeconds.toInt}
       | for j = 1, #ARGV, 2 do
       |   local offset = tonumber(ARGV[j])
       |   local value = ARGV[j+1]
       |   redis.call("SETRANGE", blobKey, offset, value)
       | end
       | local countIncrease = math.floor(#ARGV / 2)
       | local newCount = redis.call("INCRBY", countKey, countIncrease)
       | redis.call("EXPIRE", blobKey, ttlSec)
       | redis.call("EXPIRE", countKey, ttlSec)
       | if newCount >= neuronCount then
       ...
       | end
       |""".stripMargin
  private val scriptSha: String = syncCmds.scriptLoad(script.getBytes(StandardCharsets.UTF_8))
  ...
  val aggregator = Flow[(String,(Int,Double))]
    ...
    .mapAsyncUnordered(neuronCount) { batch =>
      Future {
        ...
        // use Lua script for Redis operations
        val results = batch
          .groupBy(_._1)
          .map { case (id, elems) =>
            val blobKey  = s"1_$id".getBytes(StandardCharsets.UTF_8)
            val countKey = s"cnt:1:$id".getBytes(StandardCharsets.UTF_8)
            // Prepare arguments: offset and value pairs
            val argsCount = 2 * elems.size
            val args = new Array[Array[Byte]](argsCount)
            var a = 0
            elems.foreach { case (_, (idx, dbl)) =>
              args(a) = (idx * 8L).toString.getBytes(StandardCharsets.UTF_8)
              args(a+1) = doubleBytes(dbl)
              a += 2
            }
            // Call the Lua script via EVALSHA
            val f: RedisFuture[Array[Byte]] = asyncCmds.evalsha(scriptSha, ScriptOutputType.VALUE, Array(blobKey, countKey), args: _*)
            (id, blobKey, countKey, f)
          }.toList

        asyncCmds.flushCommands()

        // collect results, compute prediction
        results.foreach { case (id, blobKey, countKey, fut) =>
          val resultBytes = fut.get()
          if (resultBytes != null) {
            val buf = ByteBuffer.wrap(resultBytes).order(ByteOrder.LITTLE_ENDIAN)
