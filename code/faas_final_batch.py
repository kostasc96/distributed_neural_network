    ...
    def run(self):
        ...
        while True:
            ...
            for message in consumer.consume():
                ...
                if batch_id not in self.batch_keys:
                    self.batch_keys[batch_id] = [None] * self.neuron_count
                    self.completed_count[batch_id] = 0
                if self.batch_keys[batch_id][neuron_id] is None:
                    self.batch_keys[batch_id][neuron_id] = f"batch:{batch_id}:n_{self.layer_id_num}_{neuron_id}"
